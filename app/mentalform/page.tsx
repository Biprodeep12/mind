...
export default function Home() {
  ...
  const [previousResults, setPreviousResults] = useState<{
    score: number;
    result: ReturnType<typeof getResultMessage>;
  } | null>(null);
    ...
    const fetchPreviousResults = async () => {
      if (user) {
        ...
        if (!querySnapshot.empty) {
          const lastSubmission = querySnapshot.docs[0]
            .data()
            .timestamp.toDate();
          ...
          const previousScore = querySnapshot.docs[0].data().score;
          const previousResult = getResultMessage(previousScore);
          setPreviousResults({ score: previousScore, result: previousResult });
        }
      }
    };
  ...
  const handleAnswer = async (optionIndex: number) => {
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      setShowResults(true);
      setPreviousResults({ score, result });
  ...
  if (!canTakeAssessment && lastSubmissionTime && previousResults) {
    const now = new Date();
    const timeDiff = now.getTime() - lastSubmissionTime.getTime();
        ...
        <div className='max-w-2xl mx-auto'>
          <Card className='p-6 shadow-lg backdrop-blur-sm bg-white/90 dark:bg-gray-800/90'>
            ...
            <p className='text-gray-600 dark:text-gray-300 text-center mt-4'>
              You have already taken the assessment. Please wait{' '}
              {hoursRemaining} more hours before taking it again.
            </p>
            <div className='mt-6'>
              ...
              <div className='space-y-4'>
                <motion.div
                  ...
                  <Shield
                    className={`h-16 w-16 text-${previousResults.result.color}-500`}
                  />
                </motion.div>
                <h2 className='text-2xl font-bold text-center text-gray-900 dark:text-white'>
                  {previousResults.result.severity}
                </h2>
                <p className='text-gray-600 dark:text-gray-300 text-center'>
                  {previousResults.result.message}
                </p>
                <motion.div
                  ...
                  <ul className='space-y-3'>
                    {previousResults.result.recommendations.map(
                      (rec, index) => (
                        <motion.li
                          key={index}
